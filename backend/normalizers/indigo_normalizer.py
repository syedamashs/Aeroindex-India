...
def normalize_indigo(raw: Dict[str, Any], task: Dict[str, Any]):
    ...
    for trip in data.get("trips",[]) or []:
      for j in trip.get("journeysAvailable",[]) or []:
        jd=j.get("designator",{}) or {}; o=jd.get("origin"); d=jd.get("destination")
        ...
        for pf in j.get("passengerFares",[]) or []:
          base=safe_float(pf.get("totalPublishFare")); taxes=safe_float(pf.get("totalTax")); total=safe_float(pf.get("totalFareAmount"))
          key=pf.get("fareAvailabilityKey"); row=blank(); row.update({
            "observation_id":stable_id("indigo",task.get("run_id"),task.get("task_id"),o,d,dep,j.get("journeyKey"),pf.get("productClass"),pf.get("FareClass"),total),
            "source":"indigo","source_url":task.get("source_url"),"search_timestamp":ts,"extraction_status":"success","currency":"INR",
            ...
            "carrier_code":",".join(dict.fromkeys([x for x in carriers if x])) or "6E","marketing_airline":"IndiGo","operating_airline":"IndiGo",
            "flight_id":j.get("journeyKey"),"journey_id":j.get("journeyKey"),"aircraft_code":",".join(dict.fromkeys(aircraft)) or None,
            "stops":safe_int(j.get("stops")) if j.get("stops") is not None else max(len(segs)-1,0),"flight_type":j.get("flightType"),
            "fare_product_class":pf.get("productClass"),"fare_class":pf.get("FareClass"),"source_offer_id":key,"fare_availability_key":key,
            "base_fare":base,"taxes":taxes,"total_fare":total,"is_sold":j.get("isSold"),"filling_fast":j.get("fillingFast"),
            "service_charges":json.dumps(pf.get("serviceCharges"),default=str) if pf.get("serviceCharges") is not None else None,
