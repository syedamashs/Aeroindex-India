...
def download_database(force_refresh: bool = True) -> Path:
    DATABASE_PATH.parent.mkdir(parents=True, exist_ok=True)
    temporary_path = DATABASE_PATH.with_suffix(".db.download")
    try:
        print(f"Fetching latest SQLite database from {DATABASE_URL}...")
        ...
        print(f"SQLite database successfully updated: {DATABASE_PATH}")
        return DATABASE_PATH
    except (HTTPError, URLError, OSError, RuntimeError) as error:
        temporary_path.unlink(missing_ok=True)
        if DATABASE_PATH.exists():
            print(f"[download_db] Warning: Failed to fetch latest database ({error}). Falling back to existing local database: {DATABASE_PATH}")
            return DATABASE_PATH
